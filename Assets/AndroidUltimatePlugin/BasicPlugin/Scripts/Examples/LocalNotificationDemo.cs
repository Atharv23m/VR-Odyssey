...
public class LocalNotificationDemo : MonoBehaviour{
	...
	public void ScheduleLocalNotification(){
		...
		localNotificationPlugin.ScheduleNotification("my notification title","my notification message","my notification ticker message",
		delay,true,true,requestCode,channelID,channelName);
	...
	public void ScheduleMultipleSpecificLocalNotification(){
		...
		for(int i =0; i < len; i++ ){
			...
			localNotificationPlugin.ScheduleSpecificNotification(requestCode + "mltiple notifcation title",requestCode,hour,minute,sec,amOrPM,
			 "my notification message_" + i,"my notification ticker message_" + i,true,true,channelID,channelName);
	...
	public void ScheduleMultipleNotification(){
		...
		for(int index=0;index<notificationCount;index++){
			...
			localNotificationPlugin.ScheduleNotification(requestCode + "notification title","my notification message",
			"notification ticker message" + index,delay,true,true,requestCode,channelID,channelName);
			notificationRequestCodeCollection.Add(requestCode);
	...
	public void ScheduleNotificationAfterDay(){
		...
		localNotificationPlugin.ScheduleAfterDay(requestCode + "notifcation title",requestCode,day,hour,minute,sec,
			amOrPM, "my notification message","my notification ticker message",true,true,channelID,channelName);

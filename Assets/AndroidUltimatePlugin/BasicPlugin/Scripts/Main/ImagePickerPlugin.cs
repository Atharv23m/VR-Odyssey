...
public class ImagePickerPlugin : MonoBehaviour {
    ...
    public void Init () {
        ...
        if (Application.platform == RuntimePlatform.Android) {
            jo.CallStatic ("init");
            isInit = true;
            SetImagePickerCallbackListener (onGetImageComplete, onGetImagesComplete, onGetImageCancel, onGetImageFail,onGetImageCropComplete,onGetImageCropFail);
    ...
    public void GetImageCrop (string cropImageDestinationPath, int width, int height) {
        ...
        if (Application.platform == RuntimePlatform.Android) {
            ...
            AUP.Utils.Message (TAG, "getImageCrop");
        } else {
            AUP.Utils.Message (TAG, "warning: must run in actual android device");
        }
#endif
